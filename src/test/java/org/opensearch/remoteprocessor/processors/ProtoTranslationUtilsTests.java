...
public class ProtoTranslationUtilsTests extends OpenSearchTestCase {
    ...
    private void setupSearchResponse() throws IOException {
        ...
        hitArray[1].sourceRef(BytesReference.fromByteBuffer(ByteBuffer.wrap("{\"field\":\"other value\"}".getBytes(StandardCharsets.UTF_8))));
        hitArray[1].shard(new SearchShardTarget("node",  new ShardId("index", "indexid", 0), null, null));
        org.opensearch.search.SearchHits hits = new org.opensearch.search.SearchHits(hitArray, new TotalHits(2, TotalHits.Relation.EQUAL_TO), 4.0f);
    ...
    public void testSearchResponseToPB_AndBack() throws IOException {
        ...
        assertEquals(this.osResponse.getInternalResponse().hits().getMaxScore(), newOsResponse.getInternalResponse().hits().getMaxScore(), 0.0000000000001);
